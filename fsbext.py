...
def setup_logging(verbose: bool, apply_console_filter: bool = True) -> None:
    """
...
def check_disk_space(
    input_dir: Path, output_dir: Path, compression_ratio: float,
    disk_usage=shutil.disk_usage, get_size_of_dir=get_size_of_dir
) -> None:
    """
    Check if there is enough disk space in the output directory based on the
    size of the input directory and a given compression ratio.
    ...
    :param compression_ratio: The ratio used to estimate the expected size
                              after extraction.
    :param disk_usage: Function to use for getting disk usage, default is
                       shutil.disk_usage.
    :param get_size_of_dir: Function to use for calculating the size of a
                            directory, default is the get_size_of_dir function
                            defined above.
    """
    ...
    if free_space < expected_size_bytes:
        logger.warning(
            "Less than {:.2f} GB of disk space available for extraction "
            "({:.2f} GB free)".format(
                expected_size_gb, free_space / (1024 * 1024 * 1024)
            )
        )
...
def create_directory_structure(output_dir: Path) -> None:
    ...
    for dir_name in directories:
        try:
            ...
            logger.error(
                f"Failed to create directory {dir_name} due to permission error: {e}"
            )
            continue
        else:
...
def remove_empty_directories(output_dir: Path) -> None:
...
def extract_and_move_files(args: argparse.Namespace, bank_files: list[Path]) -> int:
    extracted_files = 0
    ...
    for i, bank_file in enumerate(bank_files, start=1):
        ...
        try:
            ...
            logger.error(
                f"Failed to create or access directory {bank_dir} due to permission error: {e}"
            )
            continue  # Skip to the next file if the directory cannot be created
        ...
        try:
            ...
            logger.error(
                f"An error occurred while extracting {bank_file}: {e}",
                extra={"log_to_file_only": True}
            )
...
def main(args: argparse.Namespace) -> None:
    setup_logging(
        args.verbose, apply_console_filter=not args.verbose
    )
    version_str = "SKY-FSBEXT version: {} by {}".format(__version__, __author__)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Extracts audio data from sound banks in the assets folder of the "
            "video game Sky: Children of the Light, and saves them as .wav "
            "files using the vgmstream audio decoder. The extracted data can "
            "be used for game-related purposes, such as listening to the game "
            "audio outside of the game environment or for other non-commercial "
            "purposes."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-i", "--input-dir", default="in", type=Path,
        help="Path to the input directory."
    )
    parser.add_argument(
        "-o", "--output-dir", default="out", type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "Compression ratio used for calculating disk space requirements. "
            "Default is 8.0."
        )
    )
    ...
    print(
        ...
        "Please run 'python {} --help' for usage information.".format(
            Path(__file__).name
        )
    )
